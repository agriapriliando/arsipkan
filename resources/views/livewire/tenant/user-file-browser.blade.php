<div x-data="{
    ...
    <style>
        ...
        .tenant-file-filter-grid {
            display: grid;
            grid-template-columns: minmax(0, 1.5fr) minmax(12rem, 1fr) minmax(12rem, 1fr) auto;
            gap: 1rem;
            align-items: end;
        }
        ...
        .tenant-file-filter-field .form-select {
            height: 2.9rem;
    ...
    <section class="panel-box tenant-file-filter-panel mb-4">
        <div class="tenant-file-filter-grid">
            <div class="tenant-file-filter-field">
                <label for="search" class="form-label fw-semibold">Pencarian</label>
                <input id="search" type="search" wire:model.live.debounce.350ms="search"
                    class="form-control" placeholder="Cari nama file, judul, deskripsi, atau kategori">
            </div>
            <div class="tenant-file-filter-field">
                <label for="category_id" class="form-label fw-semibold">Kategori</label>
                <select id="category_id" wire:model.live="categoryId" class="form-select">
            ...
            <div class="tenant-file-filter-field">
                ...
                <select id="tag_id" wire:model.live="tagId" class="form-select">
            ...
            <div class="tenant-file-filter-actions">
                <button
                    ...
                    wire:click="$set('search', ''); $set('categoryId', ''); $set('tagId', '')"
                >
                    Reset
                </button>
            </div>
            ...
            <table class="table align-middle mb-0">
                ...
                <tbody>
                    @forelse($files as $file)
                        <tr id="file-{{ $file->id }}" wire:key="user-file-row-{{ $mode }}-{{ $file->id }}">
                            <td>
                                ...
                                <div class="text-secondary small mt-1" x-data="{ editing: {{ old('rename_file_id') == $file->id ? 'true' : 'false' }} }">
                                    @if ($mode === 'mine')
                                        ...
                                        <form method="POST"
                                            action="{{ route('tenant.user.files.original-name', ['tenant_slug' => $tenantSlug, 'file' => $file->id]) }}"
                                ...
                                <div>
                                    @if ($mode === 'mine')
                                        ...
                                        <form method="POST"
                                            action="{{ route('tenant.user.files.visibility', ['tenant_slug' => $tenantSlug, 'file' => $file->id]) }}"
                                            class="mb-1">
                                            @csrf
                                            @method('PATCH')
                            ...
                            <td class="text-end">
                                <div class="d-inline-flex gap-2">
                                    <a href="{{ route('tenant.user.files.show', ['tenant_slug' => $tenantSlug, 'file' => $file->id]) }}"
                                        class="btn btn-sm btn-light border fw-semibold">Detail</a>
                                    <a href="{{ route('tenant.user.files.download', ['tenant_slug' => $tenantSlug, 'file' => $file->id]) }}"
                                        class="btn btn-sm btn-light border fw-semibold">Unduh</a>
                                    @if ($mode === 'mine')
                                        <form method="POST"
                                            action="{{ route('tenant.user.files.destroy', ['tenant_slug' => $tenantSlug, 'file' => $file->id]) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="btn btn-sm btn-outline-danger fw-semibold"
                                                @click.prevent="openDeleteModal('{{ route('tenant.user.files.destroy', ['tenant_slug' => $tenantSlug, 'file' => $file->id]) }}', @js($file->title ?: $file->original_name))">Hapus</button>
                                        </form>
                                    @endif
                                </div>
                            </td>
