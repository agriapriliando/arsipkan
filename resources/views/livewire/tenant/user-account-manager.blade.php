<div>
    <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-end gap-3 mb-4">
        <div>
            ...
            <p class="text-secondary mb-0">Buat akun login dari uploader yang sudah pernah mengunggah file di organisasi
                ini.</p>
        </div>
    ...
    @if ($generatedPassword)
        <div class="alert alert-success d-flex flex-column gap-1">
            ...
            <div>Password: <code>{{ $generatedPassword }}</code></div>
            <div class="small text-muted">Sampaikan password ini secara manual ke uploader. User akan diminta mengganti
                password baru saat login.</div>
        </div>
    @endif

    <section class="panel-box p-4 mb-4" x-data="{
        copied: false,
        async copyLoginUrl(url) {
            try {
            ...
            } catch (error) { window.prompt('Salin URL login ini:', url); }
        }
    }">
        <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
            <div>
                ...
                <div class="small text-muted mt-1">Bagikan URL ini ke uploader organisasi agar mereka bisa login dengan
                    nomor HP dan password akun.</div>
            </div>
                ...
                <tbody>
                    @forelse($guestUploaders as $guestUploader)
                        ...
                        <tr wire:key="guest-uploader-{{ $guestUploader->id }}">
                            <td>
                                <div class="fw-bold file-name-nowrap">{{ $guestUploader->name }}</div>
                                <div class="text-secondary small">{{ $guestUploader->phone_number }}</div>
                            </td>
                            <td>
                                <div class="fw-semibold">{{ $guestUploader->files_count }} file</div>
                                <div class="text-secondary small file-name-nowrap">Terakhir diupdate
                                    {{ $guestUploader->updated_at?->translatedFormat('d M Y H:i') ?? '-' }}</div>
                            </td>
                            <td>
                                @if ($userAccount)
                                    <div>
                                        <span
                                            class="status-pill {{ $userAccount->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $userAccount->is_active ? 'Akun aktif' : 'Akun nonaktif' }}
                                        </span>
                                    </div>
                                    <div class="text-secondary small mt-1 file-name-nowrap">
                            ...
                            <td class="text-end">
                                ...
                                @else
                                    <button type="button" class="btn btn-sm btn-brand fw-semibold"
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="text-center text-secondary py-5">Belum ada uploader yang cocok
                                dengan pencarian.</td>
                        </tr>
                    @endforelse
                </tbody>
