            ...
            <section class="panel-box tenant-file-filter-panel mb-4">
                <form method="GET"
                    ...
                    <div class="tenant-file-filter-actions">
                        ...
                        <a href="{{ route($mode === 'mine' ? 'tenant.user.files.mine' : 'tenant.user.files.tenant', ['tenant_slug' => request()->route('tenant_slug')]) }}"
            ...
            <div class="table-responsive">
                <table class="table align-middle mb-0">
                    ...
                    <tbody>
                            ...
                            <tr id="file-{{ $file->id }}">
                                ...
                                <td class="text-end">
                                    <div class="d-inline-flex gap-2">
                                        <a href="{{ route('tenant.user.files.show', ['tenant_slug' => request()->route('tenant_slug'), 'file' => $file->id]) }}"
                                            class="btn btn-sm btn-light border fw-semibold">Detail</a>
                                        <a href="{{ route('tenant.user.files.download', ['tenant_slug' => request()->route('tenant_slug'), 'file' => $file->id]) }}"
                                            class="btn btn-sm btn-light border fw-semibold">Unduh</a>
                                        @if ($mode === 'mine')
                                            <form method="POST"
                                                action="{{ route('tenant.user.files.destroy', ['tenant_slug' => request()->route('tenant_slug'), 'file' => $file->id]) }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" class="btn btn-sm btn-outline-danger fw-semibold"
                                                    @click.prevent="openDeleteModal('{{ route('tenant.user.files.destroy', ['tenant_slug' => request()->route('tenant_slug'), 'file' => $file->id]) }}', @js($file->title ?: $file->original_name))">Hapus</button>
                                            </form>
                                        @endif
                                    </div>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="text-center text-secondary py-5">Belum ada berkas untuk
                                    ditampilkan.</td>
                            </tr>
                        @endforelse
                    </tbody>
